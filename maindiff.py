...
def scanCircle(image, display_image, point, radius, look_angle):
        ...
        return returnVal, data, endpoint_left, endpoint_right;
...
""".format(RESOLUTION_X, RESOLUTION_Y, SCAN_POS_X, SCAN_HEIGHT, SCAN_RADIUS_REG, NUMBER_OF_CIRCLES, SCAN_RADIUS *2)
    ...
    while True:
        ...
        returnVal, scan_data, endpoint_left, endpoint_right = scanCircle(grey_image, display_image, point_on_line, SCAN_RADIUS_REG, -90)
        ...
        for scan_count in range(0, NUMBER_OF_CIRCLES):
                returnVal, scan_data, endpoint_left, endpoint_right = scanCircle(grey_image, display_image, last_point, SCAN_RADIUS_REG, lineAngle(previous_point, last_point))
                leftpoint = leftpoint + endpoint_left[1]
                rightpoint = rightpoint + endpoint_right[1]
                # print (scan_count)
                # print('left = '+str(leftpoint))
                # print('right = '+str(rightpoint))
        ...
        print('motor100 = '+ motor100)
        print('difference = '+str(diff))
        motordiff = 100 - diff
        print ('motordiff = '+str(motordiff))
        ...
        cv2.line(display_image, (center_point[0], center_point[1]), (last_point[0], last_point[1]), red, 1)
